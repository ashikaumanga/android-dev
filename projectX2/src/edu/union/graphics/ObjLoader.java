...
public class ObjLoader extends AbstractModelLoader {
	...
	public ObjLoader(Resources resources,hand h){
		super(resources);
		this.coordinate_hand=h;
	}
	// Constructor
	public ObjLoader(Resources resources) {
		super(resources);
		this.coordinate_hand=hand.RIGHT;
	}
			...
			for (line = input.readLine(); line != null; line = input.readLine()) {
				if (line.length() > 0) {
					if (line.startsWith("v ")) {
						...
						parseMaterialLib(line.substring(7),m);
					}
				}
			}
		} catch (Exception ex) {
	...
	protected void parseMaterialLib(String line,Mesh m) {
		String resourceId = null;

		StringBuffer mtlFile=new StringBuffer(packageID);
		mtlFile.append(":drawable/");
		StringBuffer mtlName=new StringBuffer(line);
		int dind=mtlName.lastIndexOf(".");
		if(dind>-1){
			mtlFile.append(mtlName.substring(0, dind));
		}else{
			mtlFile.append(mtlName);
		}
		//System.out.println("***** "+mtlFile.toString());
		InputStream in = resources.openRawResource(resources.getIdentifier(
				mtlFile.toString(), null, null));
		// parse material file
		BufferedReader breader = new BufferedReader(new InputStreamReader(in));
		String nline = null;
		try {
			while ((nline = breader.readLine()) != null) {
				...
				if(type.equals("map_Kd")){
					if(parts.length>1){
						String mapFile=parts[1];
						StringBuffer texture=new StringBuffer(packageID);
						texture.append(":drawable/");
						StringBuffer textureName=new StringBuffer(mapFile);
						int dotInd=textureName.lastIndexOf(".");
						if(dotInd>-1){
							texture.append(textureName.substring(0, dotInd));
						}else{
							texture.append(textureName);
						}
						//System.out.println("Texture File:: "+texture.toString());

						int resId=resources.getIdentifier(texture.toString(), null, null);
						InputStream inpstr=resources.openRawResource(resId);
						TextureCache.getInstance().loadTexture(inpstr,mapFile );
						m.setTextureFile(mapFile);					
						
					}
					
				}else if(type.equals("Kd")){
					//TODO parse texture diffuse color
